...
const applyPricingLabels = () => {
  ...
  document.querySelectorAll('[data-price-route]').forEach((node) => {
    node.textContent = `$${formatPriceValue(pricingState.routePrice)}`;
  });
  document.querySelectorAll('[data-price-route-return]').forEach((node) => {
    node.textContent = `$${formatPriceValue(pricingState.routePriceReturn)}`;
  });
};
...
const loadPricingSettings = async () => {
  try {
    ...
    const routePrice = Number.parseFloat(data?.pricing?.route_price);
    const routePriceReturn = Number.parseFloat(data?.pricing?.route_price_return);
    if (Number.isFinite(hourlyRate) && hourlyRate > 0) {
      pricingState.hourlyRate = hourlyRate;
    }
    if (Number.isFinite(routePrice) && routePrice > 0) {
      pricingState.routePrice = routePrice;
    }
    if (Number.isFinite(routePriceReturn) && routePriceReturn > 0) {
      pricingState.routePriceReturn = routePriceReturn;
    }
    applyPricingLabels();
