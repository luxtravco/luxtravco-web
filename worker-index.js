...
const calcHourlyTotalCents = (hoursValue) => {
  const hours = Number.parseFloat(hoursValue);
  if (!Number.isFinite(hours) || hours <= 0) return null;
  return Math.round(HOURLY_RATE * hours * 100);
};
...
<body>
  ...
  <main>
    ...
    <table>
      ...
      <tbody>
        ${tableRows || '<tr><td colspan="17">No bookings yet.</td></tr>'}
      </tbody>
    </table>
  </main>
  <script>
...
You are Luxtravco's customer chat assistant.

Use a concise, polished, professional tone.
You are not a human. Do not pretend to be a live agent.

Business facts:
- Brand: Luxtravco
- Service: chauffeured transportation in Southern California
- Fleet: black Chevy Suburban
- Common services: airport transfers, corporate travel, events, hourly service
- Pricing examples: $79/hour, Orange County -> LAX route at $149
- Contact phone: (909) 235-0670
- Contact email: info@luxtravco.com
- If the customer asks for contact details, always provide info@luxtravco.com and (909) 235-0670
- Reservations are handled through the website booking form
- If the customer wants to book, direct them to the booking form and ask for missing details
- If the customer asks about payment, say you do not process payment in chat
- If the customer asks for a live person, say there is no live agent in chat and offer the contact details

Help with service questions, pricing, service area, vehicle details, and booking prep.
If a question is outside what you know, say so plainly and point them to the contact details.
`.trim();

const fetchChatReply = async (env, history, message) => {
  if (!env.MISTRAL_API_KEY) {
    return {
      ok: false,
      error: 'Mistral chat is not configured yet.'
    };
  }
  ...
  if (!response.ok) {
    ...
    return {
      ok: false,
      error: `Mistral request failed: ${errorText}`
    };
  }
  ...
  if (!reply) {
    return {
      ok: false,
      error: 'No reply returned by Mistral.'
    };
  }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/chat') {
      ...
      if (!message) {
        return jsonResponse({ ok: false, error: 'Message is required' }, 400, origin);
      }

      const result = await fetchChatReply(env, history, message);
      if (!result.ok) {
        return jsonResponse({ ok: false, error: result.error }, 500, origin);
      }

      return jsonResponse({ ok: true, reply: result.reply }, 200, origin);
    }
    ...
    if (url.pathname === '/api/customer/history') {
      ...
      try {
        ...
        const { results } = await env.DB.prepare(
          `SELECT *
           FROM bookings
           WHERE customer_email = ? OR customer_user_id = ?
        ...
        return jsonResponse({ ok: false, error: 'Unable to load customer bookings' }, 500, origin);
      }
    }
    ...
    const estimatedHours =
      mode === 'hourly' ? calculateRouteEstimateHours(route_points) : null;
    const hourlyTotalCents =
      mode === 'hourly' ? Math.round((estimatedHours || 0) * HOURLY_RATE * 100) : null;

    if (mode === 'hourly' && (!hourlyTotalCents || hourlyTotalCents <= 0)) {
    ...
    try {
      ...
      const insertResult = await env.DB.prepare(
        ...
        .bind(
          ...
          mode,
          mode === 'hourly' ? JSON.stringify(normalizeRoutePoints(route_points)) : '',
          mode === 'hourly' ? String(estimatedHours?.toFixed(2) || '') : '',
          mode === 'hourly' ? String(hourlyTotalCents) : '',
          mode === 'hourly' ? 'pending_payment' : 'inquiry',
          '',
          '',
          customer_email || '',
          customer_user_id || '',
      ...
      let checkoutUrl = '';
      let paymentStatus = mode === 'hourly' ? 'pending_payment' : 'inquiry';
      let stripeSessionId = '';
      ...
      if (mode === 'hourly') {
        ...
        const checkout = await createStripeCheckoutSession(env, {
          ...
          customerEmail: '',
          productName: 'Luxtravco hourly chauffeured service',
          description: `Luxtravco estimated hourly service at $${HOURLY_RATE}/hour`,
          successUrl: `${siteUrl}/?payment=success&booking_id=${bookingId}`,
          cancelUrl: `${siteUrl}/?payment=cancelled&booking_id=${bookingId}`
        });
        ...
        await env.DB.prepare(
          ...
          .bind(paymentStatus, stripeSessionId, checkoutUrl, bookingId)
          .run();
      }

      const stopsText = parseStopsText(stops);
      const slackMessage = [
        '<!channel> *New booking request*',
        `Name: ${full_name}`,
        `Mode: ${mode}`,
        ...
        mode === 'hourly' ? `Estimated time: ${estimatedHours?.toFixed(2)} hours` : null,
        mode === 'hourly' ? `Total: $${(hourlyTotalCents / 100).toFixed(2)}` : null,
        `Travelers: ${travelers || '—'}`,
        `Kids: ${kids || '—'}`,
        `Bags: ${bags || '—'}`,
